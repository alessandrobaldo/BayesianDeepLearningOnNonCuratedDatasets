...
class SGHMCSampler(_BaseSampler):
	...
	def __init__(self, loss_module, num_burn_in_steps=3000, lr=0.01, 
	...
	def sample(self, train, test, model, nsamples=1, nchains=1, **args):
...
class LossModule(nn.Module):
	def __init__(self, model,train, loss, temperature, scaling = None, non_curated = 0, corruptFunction = None, log_softmax = False):
		...
		self.non_curated = non_curated
		self.corruptFunction = corruptFunction
	...
	def getBatch(self):
		...
		if self.non_curated != 0:
			self.X, self.labels = self.corruptFunction(self.X,self.labels,self.non_curated)
	...
	def forward(self):
		self.getBatch()
		if self.log_softmax:
			outputs = self.model(self.X, log_softmax = True)
		else:
			outputs = self.model(self.X)
			
		if self.scaling == 'tempered':
			loss = (1/self.temperature)*(self.loss_fn(outputs, self.labels)) + self.negative_log_prior(self.model.named_parameters())
		elif self.scaling == 'cold':
			loss = (1/self.temperature)*(self.loss_fn(outputs, self.labels) + self.negative_log_prior(self.model.named_parameters()))
		else:
			loss = self.loss_fn(outputs, self.labels) + self.negative_log_prior(self.model.named_parameters())
		
		return loss*len(self.train)
